...
## 13. Prometheus alert + webhook 운영 (Phase 10 Stage 10.A-5)

`deploy/prometheus/alerts/multi-region.yml`에서 5개 alert rule을 노출합니다.
각 rule은 `runbook_url` annotation에 본 절(§13.1~§13.5) anchor를 포함하므로,
PagerDuty·Slack notification에서 직접 절차로 진입할 수 있습니다.

### 13.0 Alert rule 매트릭스

| Rule | 조건 | severity | 절차 |
|---|---|---|---|
| `RosshieldReplicationLagWarning` | `rosshield_replication_lag_seconds > 30` for 2m | warning | §13.1 |
| `RosshieldReplicationLagCritical` | `rosshield_replication_lag_seconds > 60` for 1m | critical | §13.2 |
| `RosshieldAuditChainHeadSeqMismatch` | tenant별 `audit_chain_head_seq` cross-instance divergence > 0 for 5m | critical | §13.3 |
| `RosshieldHARoleSwap` | `rate(rosshield_ha_failover_total[5m]) > 0` | info | §13.4 |
| `RosshieldHAFailoverStorm` | `increase(rosshield_ha_failover_total[1h]) >= 3` | critical | §13.5 |

Grafana panel은 [`deploy/grafana/rosshield-dashboard.json`](../../deploy/grafana/rosshield-dashboard.json)
HA row 참조 — `rosshield_ha_role`·`rosshield_ha_leader_epoch`·`rosshield_ha_failover_total`이
이미 panel로 노출되어 있습니다.

Alertmanager routing/receiver sample은 [`deploy/prometheus/alertmanager-sample.yml`](../../deploy/prometheus/alertmanager-sample.yml)
참조 — critical → PagerDuty + Slack #incident-response · warning → Slack #ops-alerts ·
info → Slack #audit-trail (page 없음) 권장 default.

### 13.1 `RosshieldReplicationLagWarning`

> Subscriber `{{ $labels.application_name }}` replication lag > 30s for 2m.

**1단계 — 진단 (3분 안)**

```bash
# primary PG에서 pg_stat_replication 직접 조회
sudo -u postgres psql -c "SELECT application_name, client_addr, state, sent_lsn, replay_lsn,
    EXTRACT(EPOCH FROM replay_lag) AS lag_sec
  FROM pg_stat_replication
  ORDER BY lag_sec DESC NULLS LAST;"
# Expected: state=streaming, lag_sec < 30. lag이 큰 application_name 식별.
```
...
| 상황 | 조치 |
|---|---|
| 단일 subscriber만 lag (다른 subscriber는 0~5s) | 해당 region의 네트워크·디스크 IOPS 확인 |
| 모든 subscriber lag 증가 | primary write 부하 증가 — `rosshield_scan_started_total` 추이 확인, 부하 일시적이면 wait |
| state=catchup 또는 wal_status=lost | subscription 자동 끊김 — [`multi-region-ha-setup.md`](multi-region-ha-setup.md) §subscription 재생성 절차 |

**3단계 — escalation**

10분 안에 lag이 60s를 넘으면 §13.2로 자동 promote (alert가 critical로 escalate).

### 13.2 `RosshieldReplicationLagCritical`

> Subscriber `{{ $labels.application_name }}` replication lag > 60s for 1m.
...
```
@here Replication lag critical — runbook §13.2 진입.
Subscriber: <application_name>
Lag: <seconds>
Acked by: <your-name>
Time: <UTC>
```

**2단계 — Patroni status (Patroni 환경)**

```bash
kubectl exec -n rosshield-ha rosshield-patroni-postgresql-ha-postgresql-0 -- patronictl list
# Expected: Leader 1개 + Replica 2개 모두 streaming. Lag in MB 컬럼 확인.
```

E25 환경은 [`ha-deployment.md`](ha-deployment.md)의 leader epoch · advisory lock 확인.

**3단계 — failover 결정**

| 상황 | 조치 |
|---|---|
| primary가 정상 응답 + lag만 큼 | wait + §13.1 진단 반복 — promote 보류 |
| primary 응답 timeout (write API 5xx) | §2 진단 후 §3 promote 진행 |
| Patroni 자동 cutover 진행 중 | §11 검증 절차로 전환 |

### 13.3 `RosshieldAuditChainHeadSeqMismatch`

> Tenant `{{ $labels.tenant }}` audit chain head seq가 region 간 5분 이상 차이.

**1단계 — UI 확인**

`/regions` 페이지(Stage 10.A-3 AuditConsistencyCard) → tenant 선택 → region별 head sha
표시. ❌ 표시되는 region 식별.

```bash
# 동등한 API 직접 호출
curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://primary.example.com/api/v1/audit/head-sha
curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  https://secondary.example.com/api/v1/audit/head-sha
# Expected: tenant별 head_sha + head_seq 비교. diff > 0이면 mismatch.
```
...
| 상황 | 조치 |
|---|---|
| seq diff < 5 + replication lag warning 동시 | replication 따라잡으면 자동 해결 — 5분 대기 후 재확인 |
| seq diff 큰 + replication 정상 | audit_entries replication 누락 의심 — [`audit-rotation-verify.md`](audit-rotation-verify.md) 절차로 segment 무결성 검증 |
| 한 region이 다른 region보다 seq 작음 + write 진입 중 | application route mismatch — DNS cutover 의심 |

**3단계 — 한계**

본 alert는 `rosshield_audit_chain_head_seq` 만으로 비교 — seq 일치 + sha 불일치 케이스
(replication corruption)는 잡지 못합니다. `rosshield_audit_chain_head_sha_match` 신규
gauge + cross-region collector 추가는 Stage 10.A-6 또는 별 epic에 carryover (본 doc §11
한계 참조).

### 13.4 `RosshieldHARoleSwap` (info, page 없음)

> Instance `{{ $labels.instance }}`가 leader로 승격.

**page 없음** — Slack #audit-trail 만 수신. 정기 drill 또는 rolling restart라면 무시.

예상치 못한 swap이면 다음 확인:

```bash
# 모든 instance의 HA 상태
for host in audit-{1,2,3}.acme.internal; do
  ssh ec2-user@$host "curl -s https://localhost:8080/healthz | jq '.ha'"
done
# Expected: 1개 leader + 나머지 follower. epoch 모두 동일.
```

Patroni 환경은 §11.2 Step 2 patronictl history 확인.

### 13.5 `RosshieldHAFailoverStorm`

> Instance `{{ $labels.instance }}`가 1시간 안에 3회 이상 leader 승격.

**클러스터 flapping — 즉시 대응**

**1단계 — Patroni pause (Patroni 환경)**

```bash
kubectl exec -n rosshield-ha rosshield-patroni-postgresql-ha-postgresql-0 -- patronictl pause
# 자동 failover 정지 → 안정화 후 resume
```
...
| 상황 | 조치 |
|---|---|
| etcd lease flapping | etcd 노드 health 확인 — `etcdctl endpoint health` |
| 네트워크 partition 의심 | nodes 간 ping · packet loss 측정 |
| PG load · OOM kill | dmesg + journalctl로 kernel 메시지 확인 |

**3단계 — 안정화 후 resume**

```bash
kubectl exec -n rosshield-ha rosshield-patroni-postgresql-ha-postgresql-0 -- patronictl resume
# 또는 ROSSHIELD_HA_RP=e25 fallback (§11.5)
```

postmortem 필수 (§9.3 template).
