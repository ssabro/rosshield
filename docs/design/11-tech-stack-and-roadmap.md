...
> **결정: Go 백엔드 + TypeScript 프론트 (2026-04-23 확정).** 근거는 본 절과 §11.16 결정 로그.
...
### 체크포인트 결정 — 종결 (2026-04-23)

원안에서는 Stage 1 시작 전 2주 스파이크(Go·TS 각각 최소 기능 비교)를 권고했으나, 설계서의 3종 배포·에어갭·단일 바이너리 요구가 Go 쪽으로 명확히 기울어 **스파이크 생략**하고 Go를 곧바로 채택. TS는 프론트 전담으로 분리.
...
> **결정: Tauri 2.x (2026-04-23 확정).** Go 서버는 자식 프로세스로 띄우고 Tauri는 얇은 WebView 껍질만 담당. Electron은 Phase 1 긴급 출시 사유가 생길 때만 fallback.

| 후보 | 평가 |
|---|---|
| **Tauri 2.x** ✅ | 번들 작음(수십 MB), Go 서브프로세스 방식으로 독립 가능, WebView2/WKWebView/WebKitGTK 활용. **채택**. |
| Electron | `nrobotcheck` 자산 재활용 최대. 번들 큼(150MB+), 네이티브 모듈 ABI 고민. fallback 보류. |
| Wails | Go 네이티브 친화. Tauri 대비 생태계 작음. |
| .NET MAUI | Windows 주력이면 가능. 크로스 플랫폼 성숙도 중간. |

## 11.9 어플라이언스 OS

> **결정 연기 (2026-04-23).** Phase 4는 1년+ 이후이고 어플라이언스 OS 생태계 변동 가능성이 있어 **Phase 3 exit 시점에 최종 확정**. 그때까지 본 설계서·로드맵은 Ubuntu Core 24를 **기본 가정**으로 유지.

| 후보 | 평가 |
|---|---|
| **Ubuntu Core 24** (기본 가정) | snap 기반 immutable. 검증된 생태계. |
| Fedora IoT / OSTree | 원자적 업데이트 강력. 유지 주체 이슈 모니터링 필요. |
| Debian + custom | 유연하지만 직접 관리 부담. |
| Talos Linux | K8s 어플라이언스에 최적. 과잉. |

기본 가정 경로: Core 컴포넌트는 snap으로, 추가 서비스는 OCI 컨테이너(snap내 podman)로 감싸 A/B 슬롯 업데이트.
...
- **아티팩트**:
  ...
  - `rosshield-desktop` (Tauri, 3 OS)
  - OCI 이미지 (`ghcr.io/org/rosshield:1.3.2`)
  - 어플라이언스 스냅·이미지
...
- [ ] **enterprise build tag 골격** — `internal/enterprise/{crosswitness,selectdisclose,multihash,wasmrt,robotid,rostopo,fleetxval}/` + `//go:build rosshield_enterprise` 가드. D8 1순위 결합 청구항(A-1·B-1·C-1·D-3) 구현을 위한 분리 베이스. D5 원안 "첫 paying customer 직전" 분리에서 가속. 상세: `13-patent-strategy.md` §13.3.
...
- [x] **D1 — 제품명 확정**: **Lodestar** 확정(2026-05-18, D-P7-1). 코드 네임스페이스 `rosshield` 확정(2026-04-23, Google 검색으로 무충돌 검증). 제품 브랜드 Lodestar는 Top 3 후보 중 등록 가능성 최우선 — 보안·SW 분야 dominant player 부재, "신뢰의 기준점" 메타포가 외부 감사인 검증 가치와 직접 공명. 초기 가칭 "FleetGuard"는 Cummins·Attestor.ai 등 상표 충돌로 폐기.
- [x] **D2 — 백엔드 언어**: **Go** (+ TS 프론트) · 2026-04-23. 근거: 단일 정적 바이너리로 3종 배포(데스크톱 Tauri 자식 프로세스 / Compose / 어플라이언스) 전부 natural fit, `crypto/ssh`·`ed25519` 성숙, P3(에어갭)·P7(단일 바이너리)에 최적. `nrobotcheck`의 Electron·better-sqlite3 ABI 운영 부담 회피.
- [x] **D3 — 데스크톱 셸**: **Tauri 2.x** · 2026-04-23. Go 서버는 자식 프로세스, Tauri는 얇은 WebView 껍질. Electron은 fallback.
- [ ] **D4 — 어플라이언스 OS**: 연기(2026-04-23). 기본 가정 **Ubuntu Core 24**, Phase 3 exit 시점에 최종 확정.
- [~] **D5 — 라이선스 모델**: open-core 골격은 유지(2026-04-23). 단 코어 라이선스(Apache-2.0 vs BSL/Elastic License)와 코어/엔터프라이즈 코드 분리 시점은 Phase 3 진입 시 재논의(2026-05-07 재검토 결정). 근거: HashiCorp Vault·Elastic 사례에서 보듯 처음부터 permissive로 시작하면 AWS/대기업 SaaS화 견제 불가. enterprise 기능(SSO·MT·webhook·라이선스)이 들어오는 Phase 3 진입 시점에 BSL/Elastic License 같은 보호 라이선스 적용 + 코어/엔터프라이즈 분리 검토.
- [~] **D6 — 리포 호스팅**: 2026-05-07 보류 결정. Phase 1 exit 후 public 전환 계획은 보류 — Phase 3 진입 시 D5(라이선스) 결정 후 재논의. 사용자(2026-05-07): "상업적 도구로 만드는데 핵심 기술이 공개되는 게 우려됨". P1(외부 검증 가능성)은 public 없이도 "감사인이 빌드 검증 가능한 결정론적 결과 + 검증된 contributor에게 read 권한 부여"로 대체 가능. **2026-05-08 R30-4로 D5/D6 모두 확정**(CLAUDE.md 반영) — D5 = Open-core(코어 Apache-2.0 + enterprise 별 라이선스) + 단일 repo + build tag, D6 = GitHub private 유지(verify CLI E30로 P1 대체).
- [x] **D8 — 특허 전략**: 2026-05-08 확정. (1) 후보 D(ROS2 도메인 특화) 전체 신규 채택 + 후보 A·B·C 강화안 + 후보 E는 Phase 4 어플라이언스 진입 시 검토(보류). (2) 1순위 결합 청구항 = A-1(cross-witness fold-in) + B-1(multi-hash evidence) + C-1(WASM sandboxed evaluator) + D-3(robot identity binding: TPM EK + MAC + CPU serial fingerprint). (3) Open-core 청구 분배 원칙 — Apache-2.0 §3 patent grant 회피를 위해 핵심 알고리즘은 `internal/enterprise/*` build tag 코드에 배치, 코어에는 인터페이스·verify CLI만. (4) enterprise build tag 골격을 Phase 4 항목으로 가속 도입(§11.13 Phase 4). (5) 출원 전 GitHub public 전환·LICENSE 추가·외부 공개(컨퍼런스/블로그/논문)·NDA 없는 PoC 모두 금지. 산출물: `docs/design/13-patent-strategy.md` + `docs/ip/spec-candidate-A-draft.md`. **다음 단계**: 변리사 컨설팅 → 선행기술 조사 → 04/07/08/10 본체 갱신 → 명세서 최종화 → KR 출원 → 12개월 내 PCT 평가.
