...
_마지막 업데이트: 2026-04-23 (E1.T3 IDGen 완료)_

---

## 현재 상태 한 줄

**Phase 1 E1 진행 중 — T1·T2·T3 완료, T4~T9 대기.** `internal/platform/idgen/` `IDGen` 인터페이스(`New(prefix) string`) + `NewULID()` (oklog/ulid v2 + crypto/rand + sync.Mutex, monotonic ms-내 ordering), 5 tests pass. 원격 12개 커밋, CI green in 1m36s. 다음 세션 착수 후보: R1·R2 노트 "미해결 질문" 합의 미팅 → E1.T4 Storage.
...
E1.T1·T2·T3 완료 + R1·R2 사전 설계 노트 확보 상태에서 재개 후보:

1. **R1·R2 노트의 "미해결 질문" 결정 미팅** (권장) — 각각 7건씩 총 14건. E1.T4(Storage)·E1.T6(EventBus) 착수 전 합의 필요. 주요 항목:
...
2. **E1.T8 Signer 착수** — `crypto/ed25519` 메모리 키 wrapper(파일 저장은 E2). 노트 불필요, T4·T6 합의 전 진행 가능한 작은 태스크.
3. **E1.T9 Scheduler 착수** — `robfig/cron/v3` 기반. T2 Clock 의존(이미 완료), 단 결정론적 테스트가 필요하면 Clock 인터페이스 확장 검토.
...
6. **로컬 환경 정리** — `cmd/rosshield-server` 테스트 실행 시 Windows Defender가 `%TEMP%\go-build\*.test.exe`를 격리하는 현상(2026-04-23). CI(Linux)는 영향 없음. Defender 예외 추가 또는 `GOTMPDIR` 변경으로 우회 가능.

**권장 순서**: 1(미해결 질문 결정) → E1.T4 Storage → E1.T6 EventBus → E1.T5/T7 → E1.T8 Signer → E1.T9 Scheduler. T8·T9는 독립적이라 합의 대기 중에 인터럽트로 처리 가능.
