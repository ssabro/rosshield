...
E6 SSH+Scan 진행 중. Stage A·B·C·D.1 완료, Stage D.2·D.3 대기.

1. **E6 Stage D.2 진입 (권장)** — bootstrap 결선. `cmd/rosshield-server/bootstrap.go`에 (a) `sshpool.Executor` → `scan.SSHExecutor` 어댑터 (robot.Service.GetCredentialMaterial 호출 결합) + (b) `benchmark` evaluator → `scan.CheckEvaluator` 어댑터 + (c) `Platform.ScanRun *scanrun.Orchestrator` 결선. in-proc fakesshd 확장으로 통합 테스트 (T7 evaluation rule fixture). 추정 2일.
