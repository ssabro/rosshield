...
E6 SSH+Scan 진행 중. Stage A·B·C 완료, Stage D·E 대기.

1. **E6 Stage D 진입 (권장)** — Orchestrator 결선. `internal/domain/scan/` 또는 별도 application 서비스에 `Orchestrator.Run(ctx, session, robots, checks)`: pending→running 전이 + worker pool 10 + queue=robot×check 카티전 곱 + 각 worker가 sshpool.Executor로 SSH exec + E4 sealed AST evaluator로 평가 + scan.Service.RecordResult 호출 + EventBus 진행률 publish + cancel 시 R4-5 시멘틱(진행 중 완료대기·다음 item 스킵). E6.T5~T10 모두 여기서. 약 1주.
