...
_마지막 업데이트: 2026-04-23 (Phase 0 전면 종료 — 원격 `ssabro/rosshield` PRIVATE + CI green)_

---

## 현재 상태 한 줄

**Phase 0 완료, Phase 1 착수 대기.** 설계서 13종 + Phase 1 백로그 + D1~D6 결정 + 코드네임 `rosshield` + Go 1.26 모듈 + `/healthz` TDD 스텁 + OpenAPI 3.1 스켈레톤 + **GitHub origin 연결 + CI workflow green(vet·build·test·golangci-lint v2)**. 다음은 Phase 1 E1(Platform L1) TDD 착수.
...
Phase 0 완전 종료 + 원격 연결 + CI green. 다음 세션이 고를 경로:

1. **Phase 1 E1 착수 — Platform L1** (권장). `docs/design/phase1-backlog.md` E1.T1(Logger 구조화 필드)부터 TDD Red→Green. 이후 E1.T2~T9(Clock·IDGen·Storage·EventBus·Signer·Scheduler) 순차.
2. **Step 0.3-β — OpenAPI 코드 생성 파이프라인** — `oapi-codegen` 설치, `Makefile openapi` 타깃 실구현(`oapi-codegen -package api -generate types,chi-server openapi/openapi.yaml`), spectral 린트 CI. E9 API 실장 전 한 번만 하면 반복 이득.
3. **depguard 도메인 경계 린트 설정** — `.golangci.yml`에 L1→L2→L3→L4 계층·도메인 간 import 차단 규칙. Phase 1 E3 전에 깔아두면 경계 위반이 린트로 즉시 차단.
4. **설계서 보강** — §08 intelligence 심화, §06 KEK/DEK 세부, §07 SCAP 변환 경로 등 특정 섹션 추가 확장.
5. **휴식 / 세션 종료** — 자연스러운 break point.

**권장 순서**: 1을 전체 흐름의 중심에 두고, 3은 1의 중간(E1~E2 사이)에 삽입, 2는 1이 E8 근처 진행될 때 정식 도입.
