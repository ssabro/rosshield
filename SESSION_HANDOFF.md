...
_마지막 업데이트: 2026-04-28 (E6 Stage D.3 완료 → **E6 epic 종료** — 통합 검증 + goleak + sshpooltest export)_
...
**E6 epic 완전 종료** (Stage A·B·C·D.1·D.2·D.3 + T1~T10). Phase 1 backlog 12 epic 중 ~6개 완료(E1~E6 코어).

1. **E12 pack-tools 진입 (권장)** — `cmd/pack-tools convert` — nrobotcheck 312+329 baseline → rosshield pack 형식 변환. 백그라운드 agent가 4계층 evaluation 패턴 조사 완료. CIS Ubuntu 팩으로 감사가 Phase 1 Exit 조건이라 critical path. 추정 1주.
2. **E7 Evidence 진입** — Content-addressed Blob Store(파일어댑터, sha256 dedupe·refcount). E6 ScanResult가 evidence 해시로 고정되므로 자연 후속. 추정 3일.
