...
test('admin login redirects out of /login and reveals authenticated shell', async ({ page }) => {
  ...
  // D-P7-1: 헤더 UserMenu dropdown trigger를 열고 menuitem '로그아웃' 노출 확인.
  await page.getByRole('button', { name: KO_LABELS.header.userMenu }).click()
  await expect(page.getByRole('menuitem', { name: KO_LABELS.header.logout })).toBeVisible()
  // 사용자 이메일이 dropdown 안에 노출되는지 (text 부분 일치).
...
test('logout clears session and redirects to /login', async ({ page }) => {
  ...
  // UserMenu dropdown 열고 menuitem '로그아웃' 클릭.
  await page.getByRole('button', { name: KO_LABELS.header.userMenu }).click()
  await page.getByRole('menuitem', { name: KO_LABELS.header.logout }).click()
