...
// TestHotGC_SetLocalRejectedOnSQLite — sqlite는 SET LOCAL 미지원 — 후보 segment 가 있고
// dryRun=false 면 첫 SET LOCAL Exec 에서 syntax error 로 차단됨을 검증 (PG 미적용 보호막).
//
// PG 환경에서는 본 path 가 정상 통과 — integration test 에서 실측.
