// Package robotid implements D8-D3 robot identity binding (TPM EK + MAC + CPU
// serial 결합 fingerprint) — enterprise edition.
//
// 본 패키지는 enterprise build tag(`rosshield_enterprise`) 안에서만 실 구현이
// 빌드됩니다. 코어 빌드에서는 robot ID(`rb_<ULID>`) 단일 식별자만 사용합니다.
//
