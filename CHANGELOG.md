...
### Added
- (placeholder) 차기 release 항목 — Phase 12 backlog draft 진입 / enterprise plugin 잔여 (`fleetxval` · `rostopo` · `selectdisclose`) / testcontainers e2e Jaeger smoke (v0.14.0 carryover) / otel metric exporter plug (D-P11A-5 양쪽 emit 중 otel metric 부분, v0.14.0 carryover) / Lodestar Grafana dashboard panel — trace volume + sampling effective rate + cross-region trace coverage (v0.14.0 carryover) / Lodestar UI 안 trace_id 표시 + Jaeger/Tempo deep link (v0.14.0 carryover) / audit chain head sha mismatch metric (Phase 10.A-6 carryover) / manual rotation endpoint / multi-tenant epoch 분리 / Grafana dashboard panel (rotation_total + key_epoch + hash_version, v0.13.0 carryover) / fg-verify v3 binary 외부 감사인 분배 (★ 외부 트랙, D-P11C-4, v0.13.0 carryover) / humble pack archive embed (PACKS_SOURCE Makefile humble 등록 + `_archives/` 자동 embed, v0.11.0 carryover) / soc2-controls pack archive embed (PACKS_SOURCE Makefile 등록, v0.12.0 carryover) / SROS2 cert chain intermediate CA + OCSP responder (v0.11.0 carryover) / SROS2 keystore 자동 enrollment workflow / DDS topic whitelist expansion (`/diagnostics` · `/rosout` · custom payload, v0.11.0 carryover) / A4 Privacy GDPR/CCPA/한국 PIPA 상세 매핑 (v0.12.0 carryover) / DC physical security (CC6.4) + environmental monitoring (A1.3) IaaS cloud provider SOC2 inheritance 매핑 (v0.12.0 carryover) / Phase 9.5 testcontainers e2e Patroni 3-node + etcd / C5b-10 a11y polish Tailwind palette contrast / MR.T4 application restart integration / Stage 4.5 BIND/PowerDNS Terraform sample (ops doc cover) / Stage 5b 잔여 carryover (C5b-6/C5b-7/C5b-8/C5b-9) / R-D8 청구권 명세서 (사용자 외부) / E36 레퍼런스 HW burn-in (사용자 hands-on)

---

## [0.14.0] — 2026-05-22 (minor — Phase 11 옵션 A OpenTelemetry tracing 전면 마감)

> **요약**: Phase 11 옵션 A "OpenTelemetry tracing 전면" 완전 마감 + Phase 11 세 번째(마지막) minor release — **Phase 11 Top 3 순차 B → C → A 모두 마감**. Lodestar observability 자산이 Prometheus metrics + Grafana dashboard + structured slog + audit chain 위에 분산 trace 채널까지 확장. `internal/platform/otel/` SDK provider scaffold (parent_based(traceidratiobased(0.05)) default sampler + W3C TraceContext + Baggage composite propagator + OTLP gRPC/HTTP exporter both, D-P11A-2) + HTTP middleware (request trace + W3C `traceparent` header propagation) + scan flow 5 span (`scan.ssh.connect` · `scan.check.exec` · `scan.check.evaluate` · `scan.evidence.write` · `scan.publish`) + multi-region + Patroni REST trace propagation + `httpclient.WrapClient` / `WrapTransport` helper (outbound HTTP `traceparent` 자동 inject — patroni · webhook · LLM 3 provider 결선) + LLM 4 provider span (noop · anthropic · ollama · vllm, `llm.complete` span + token/cost/duration attribute, **PII 회피 엄격**: prompt/response/tool args 미노출) + 5 CLI flag + env fallback (`--otel-enabled` · `--otel-endpoint` · `--otel-exporter` · `--otel-sampling` · `--otel-insecure`) + `docs/operations/opentelemetry-setup.md` 신규 + `docs/onboarding/otel-collector-setup.md` 신규. 운영 default = **opt-in disabled** (`Enabled=false`, noop tracer, overhead 0) — air-gap customer 영향 0. 단위 test 30+ 신규 (50+ 패키지 회귀 0). 코드 영향 최소 (docs + scaffold 위주). 상세는 [docs/releases/v0.14.0.md](docs/releases/v0.14.0.md).
>
> **기준 commit**: (본 release commit, main)

### Added
- `design(phase11-a)` Phase 11 옵션 A Stage 분해 + 결정 항목 (`3ded42a` + `03d1bba`) — `opentelemetry-tracing-design.md` 신규 (12 섹션 + 8 영역 fact-check + 4 옵션 비교) + D-P11A-1·2·3·4·5 결정 확정 (옵션 A otel SDK 전면 / grpc+http both / parent_based 5% / Loki 권장 default + customer 선택 / Prometheus federation + otel exporter 양쪽 plug).
- `feat(otel)` Phase 11.A-2 OpenTelemetry SDK + provider scaffold + bootstrap 결선 (`1117641`) — `internal/platform/otel/` 신규 패키지 (`Provider` + `Config` + `NewProvider` + `NewSampler` + `newExporter` + `newResource` + `NewProviderFromComponents`) + parent_based(traceidratiobased(0.05)) default sampler + W3C TraceContext + Baggage composite propagator + OTLP gRPC/HTTP exporter both (D-P11A-2) + service.name + region + environment resource attribute + `Provider.Shutdown` graceful (default 5s) + bootstrap `buildOtelProvider(ctx, cfg)` helper + `Deps.Otel` field 노출 + 5 CLI flag + 5 env fallback (`--otel-enabled` + `--otel-endpoint` + `--otel-exporter` + `--otel-sampling` + `--otel-insecure`) + 12 단위 test (noop short-circuit + endpoint validation + ratio 정규화 + exporter selection + resource attribute + propagator composite + shutdown idempotent + components injection).
- `feat(otel)` Phase 11.A-3 HTTP middleware (request trace + W3C traceparent propagation) (`e1da604`) — `internal/api/handlers/otel_middleware.go` 신규 + `OtelTrace(provider)` middleware + chi router `apiRouter.Use(handlers.OtelTrace(p.Otel))` 결선 + incoming `traceparent` parse (있으면 parent 상속, 없으면 root 신규) + span name `HTTP <METHOD> <route>` (chi route pattern) + `http.method` + `http.route` + `http.status_code` + `rosshield.tenant_id` attribute (cross-tenant 분리 필수, R11A-3) + noop short-circuit (응답 latency 영향 0) + 8 단위 test (incoming traceparent parse + root span emit + status_code 부착 + tenant_id 부착 + noop short-circuit + middleware 순서 정렬: `RequireLeaderForWrites` · `StandbyReadOnlyMiddleware` 보다 앞).
- `feat(otel)` Phase 11.A-4 scan flow 5 span instrument (`3cf37e2`) — `internal/app/scanrun/scanrun.go` + `internal/app/scanrun/scanrun_span.go` 신규 + `scan.run` root span + 5 child span (`scan.ssh.connect` · `scan.check.exec` · `scan.check.evaluate` · `scan.evidence.write` · `scan.publish`) + `rosshield.tenant_id` + `rosshield.scan_id` + `rosshield.fleet_id` + `rosshield.pack_id` + `rosshield.robot_id` + `ssh.host` + `ssh.exit_code` + `check.id` + `check.category` + `check.severity` + `check.outcome` + `evidence.byte_count` + `evidence.format` + `audit.event.action` + `eventbus.topic` attribute + ctx propagation 정확성 (worker goroutine) + Tracer nil 시 noop fallback + 5 단위 test (in-memory `tracetest.SpanRecorder` 기반 5 span tree + attribute 검증 + noop short-circuit).
- `feat(otel)` Phase 11.A-5 multi-region + Patroni REST trace + `httpclient.WrapClient` helper (`ea10153`) — `internal/platform/httpclient/otel.go` 신규 + `WrapClient(client, tp, target)` + `WrapTransport(base, tp, target)` + outbound `traceparent` 자동 inject + `rosshield.outbound.target` attribute (`patroni`/`webhook`/`llm`/`anchor`) + tp == nil 또는 Enabled=false 시 short-circuit + multi-region 4 replication endpoint (heartbeat · failover · status · health) incoming HTTP span + Patroni REST adapter outbound HTTP span 자동 연결 (cross-region distributed trace 완성) + bootstrap 3 위치 결선 (Patroni adapter `Tracer: otelProvider.Tracer("rosshield/platform/ha/patroni")` + webhook `httpclient.WrapClient(..., "webhook")` + 4 LLM provider `httpclient.WrapClient(..., "llm")` 일괄) + 6 단위 test (Wrap noop short-circuit + outbound traceparent inject + target attribute 부착 + client field 보존 + transport nil fallback).
- `feat(otel)` Phase 11.A-6 LLM 4 provider span (`10e92d4`) — `internal/platform/otel/llm.go` 신규 + `internal/app/advisorrun/llm_client.go` 결선 + `LLMSpanInput` + `LLMSpanOutcome` struct + `LLMInputAttributes` + `LLMOutcomeAttributes` 단일 진실 source (4 provider 동일 형식 보장) + `llm.complete` span (scope `rosshield/advisorrun/llm`) + attribute `llm.provider` + `llm.model` + `llm.tool_count` (size only) + `llm.tokens.input` + `llm.tokens.output` (size only) + `llm.cost.usd` + `llm.duration.ms` + `llm.stop_reason` + `llm.error` + `llm.disabled` (noop 경로 식별) + **PII 회피 엄격** (D-LLM-3 + design §6.6 — prompt/response/tool name/tool args 모두 attribute 포함 0, token count + cost + duration + provider/model 식별자만) + 4 provider 각각 `httpclient.WrapClient(client, otelProvider, "llm")` outbound transport wrap (Stage 11.A-5 helper 재사용) + `advisorrun.NewLLMClientWithTracer(llmAdapter, otelProvider.Tracer(LLMTracerScope))` 별 생성자 (기존 NewLLMClient 시그니처 변경 0) + 6 단위 test (span emit + attribute 매핑 + error span status + noop short-circuit + PII 회피 prompt/response 부재 검증).
- `docs(release)` Phase 11.A-7 v0.14.0 release notes + ops docs + CHANGELOG entry (본 release commit) — `docs/releases/v0.14.0.md` 신규 + `docs/operations/opentelemetry-setup.md` 신규 (운영자 가이드 11 섹션: 개요 + 빠른 시작 + Backend 선택지 8종 + Trace 분석 + Sampling 정책 + Log aggregation 옵션 + PII 회피 정책 + 한계) + `docs/onboarding/otel-collector-setup.md` 신규 (customer onboarding 11 섹션: docker-compose Jaeger + Tempo+Grafana stack + Datadog + NewRelic + Snap + Kubernetes sidecar + tail-based sampling + 검증 체크리스트) + 본 entry.

### Changed
- `cmd/rosshield-server/bootstrap.go` — `buildOtelProvider(ctx, cfg)` helper + `Deps.Otel` field + provider Shutdown graceful 흐름 + 4 결선 위치 추가 (chi router otel middleware + scanrun Tracer 주입 + Patroni adapter Tracer 주입 + webhook/LLM `httpclient.WrapClient` outbound wrap).
- `internal/platform/logger/logger.go` — 기존 `contextHandler` 가 span context 에서 `trace_id` 자동 추출 (Phase 11.A-2 결선 활용, 추가 코드 0). Loki/ELK/Splunk 등 log aggregation backend 에서 단일 trace_id 로 log+trace cross-reference.
...
### Verification
- `go vet ./...` PASS · `go build ./internal/... ./cmd/...` PASS · `go test -count=1 ./...` PASS (50+ 패키지 회귀 0).
- `go test -count=1 ./internal/platform/otel/...` PASS (12 provider + 6 llm + 6 sampler/resource/exporter).
- `go test -count=1 ./internal/platform/httpclient/...` PASS (6 WrapClient/WrapTransport).
- `go test -count=1 ./internal/api/handlers/...` PASS (otel middleware 8 + RBAC 회귀 0).
- `go test -count=1 ./internal/app/scanrun/... ./internal/app/advisorrun/...` PASS (scan 5 span + llm.complete span + in-memory SpanRecorder).
- 회귀 0 — Prometheus collectors 14종 + Grafana dashboard 영향 0 (D-P11A-5 backward compat) + audit chain (v1/v2/v3) 영향 0 (R11A-4) + multi-region replication + Patroni 자동 failover 회귀 0 (Enabled=false 시 short-circuit) + LLM 4 provider Complete/CompleteStream 시그니처 변경 0.

### Notes
- **minor bump 이유** — Phase 11 세 번째(마지막) minor (Phase 11 옵션 B v0.12.0 + 옵션 C v0.13.0 후속). Lodestar observability 자산에 분산 trace 채널 신규 추가 + 5 CLI flag + 5 env + 4 instrumented hot path + 2 ops docs = customer-facing 신규 기능 묶음.
- **customer 영향 0 (opt-in default)** — `--otel-enabled=false` (default) 시 noop tracer, exporter 미연결, span emit 0, transport wrap 0, 응답 latency 영향 0. 미활성 customer + air-gap customer 운영자 알람 없음.
- **Phase 11 Top 3 모두 마감** — 옵션 B (SOC2 readiness · v0.12.0) ✅ + 옵션 C (audit hash key_epoch + fg-verify v3 · v0.13.0) ✅ + 옵션 A (OpenTelemetry tracing 전면 · v0.14.0) ✅. → Phase 12 backlog draft 진입 가능 또는 enterprise plugin 잔여 (`fleetxval` · `rostopo` · `selectdisclose`) 진입 가능.
- **PII 회피 엄격** — prompt/response content · tool name/args · ssh credential · evidence content · audit event metadata · HTTP request body 모두 span attribute 노출 0. token count + cost + duration + provider/model 식별자 + tenant_id (cross-tenant 분리 필수) 만 노출. customer 보안 팀 검증 가능.
- **vendor-neutral** — D-P11A-1 옵션 A 일관. vendor-specific APM SDK (Datadog · NewRelic · AppDynamics · Dynatrace) 직접 통합 거부. customer 측 otel-collector 경유 (Jaeger/Tempo/Datadog/NewRelic 8종 backend 가이드 제공). lock-in 회피.
- **신규 carryover** — testcontainers e2e Jaeger smoke (CI docker pull 비용 회피, 별 epic) / otel-collector + backend customer 위탁 (★ 환경 의존) / log aggregation (Loki/ELK/Splunk/Datadog Logs) customer 환경 의존 / vendor-specific APM 직접 통합 미지원 (otel-collector 경유 권장) / otel metric exporter plug 미결선 (Phase 12+ 후보) / Lodestar Grafana dashboard panel 별 dashboard epic / Lodestar UI 안 trace_id 표시 별 epic.

### Migration
- 신규 v0.14.0 deploy 시 `--otel-enabled=false` (default) — 변경 없이 그대로 작동 (overhead 0).
- customer 측 otel-collector + backend (Jaeger/Tempo/Datadog/NewRelic) 사전 설치 후 `--otel-enabled=true --otel-endpoint=otel-collector:4317 --otel-sampling=0.05` 활성. `docs/onboarding/otel-collector-setup.md` 의 docker-compose 예제 참조.
- air-gap customer 는 자체 air-gap collector 가 있으면 `--otel-insecure=true` 활성 가능. backend 가 없으면 그대로 disabled (noop, overhead 0).
- sampling 조정 — low-volume (1.0), 일반 (0.05 default), high-volume (0.01), tail-based collector 결합 (Lodestar 측 1.0 + collector 정책).

---

## [0.13.0] — 2026-05-22 (minor — Phase 11 옵션 C audit hash key_epoch + fg-verify v3 마감)

> **요약**: Phase 11 옵션 C "audit hash chain key_epoch + leader_epoch input + fg-verify v3" 완전 마감 + Phase 11 두 번째 minor release (Phase 11 Top 3 순차 B → C → A 진척 중, v0.13.0 옵션 C 2순위 마감, v0.12.0 옵션 B 후속, v0.14.0 옵션 A 진입 가능). `canonicalMetaJSONv3` 신규 (9 키 알파벳순 — 기존 7 키 + `keyEpoch` + `leaderEpoch`) + `ComputeEntryHashV3` hash 함수 + internal `computeEntryHash` dispatcher (HashVersion V1·V3 enum) + `audit.chain.hash_version_changed` transition marker entry (idempotent bootstrap emit, `RegisterCheckpointJob` 직후 1 Tx 안) + `Repo.Append` 경로 hash version 분기 (`transitionSeq atomic.Int64` cache, `entry.Seq > transitionSeq` 만 v3) + `Repo.ExportV3` + `_bundleVersion: "v3"` + `_hashVersionTransitionAt` + `LeaderEpoch` 노출 + `MarshalEntryLineV3` (v2 wire byte-identical 보존, LeaderEpoch는 v3만 노출) + `fg-verify` v3 verify (`selectHashFunc` + `verifyHashChainForBundle` + `verifyHashVersionTransitionMarker` + v1/v2/v3 3-tier backward compat 엄격) + testdata v3 fixture (deterministic chacha8 seed, 1079B, 5 entries) + 단위 test 5 신규 + metric `audit_chain_hash_version` Gauge + `audit_chain_hash_version_transition_total` Counter + `docs/operations/audit-verify-cli.md` v3 절차 갱신. DB 마이그레이션 0건 (Phase 10.D-3+4 결선 컬럼 재활용). 외부 감사인 fg-verify v3 binary 분배 (D-P11C-4 = 본 release 후 시점) — v0.10.0 carryover ("audit hash chain key_epoch+leader_epoch input 미포함") 완전 마감. 회귀 0 (v1/v2 fixture 그대로 PASS, v2 wire byte-identical 보존), Breaking 0, customer 영향 0 (v1/v2 bundle 자동 분기 + v3는 자연 transition). 상세는 [docs/releases/v0.13.0.md](docs/releases/v0.13.0.md).
>
> **기준 commit**: (본 release commit, main)

### Added
- `design(option-c)` Phase 11 옵션 C Stage 분해 + 결정 항목 (`35f80d0` + `67f73cf`) — `audit-hash-key-epoch-input-design.md` 신규 (535줄 + 11 섹션 + 4 옵션 비교) + D-P11C-1·2·3·4 결정 확정 (옵션 A canonicalMetaJSON keyEpoch + leaderEpoch input + fg-verify v3 3-tier / 기존 v1 hash 유지 + transition marker entry / keyEpoch + leaderEpoch 둘 다 / Stage 11.C-6 release 후 fg-verify v3 binary 분배).
- `feat(audit)` Phase 11.C-2 canonicalMetaJSONv3 hash (`29c679d`) — `ComputeEntryHashV3` 신규 (9 키 알파벳순: `action` < `actor` < `keyEpoch` < `leaderEpoch` < `occurredAt` < `outcome` < `seq` < `target` < `tenantId`) + `canonicalMetaJSONv3` 신규 (`keyEpoch`/`leaderEpoch *int64 omitempty` 직렬화) + `HashVersion` enum (V1=1, V3=3) + 내부 `computeEntryHash` dispatcher + `canonicalMetaJSONv1` (기존 함수 rename, backward compat 엄격) + 단위 test (nil-epoch byte-identical v1==v3 + epoch set 시 v3≠v1 + value 변경 민감도 + determinism + 9 키 field mutation + wire-format 알파벳순 sanity + sha256 wire-reconstruction cross-check).
- `feat(audit)` Phase 11.C-3+4 hash version transition marker + Append v3 분기 + ExportV3 (`2b6e987`) — `audit.ActionHashVersionChanged = "audit.chain.hash_version_changed"` 상수 + `HashVersionTransitionMeta` payload + `audit.EnsureHashVersionTransition` (idempotent emit + setter cache) + `audit.HashVersionLocator` + `audit.HashVersionTransitionSetter` interface (도메인 경계 P5 일관) + bootstrap `ensureHashVersionTransition` (`cmd/rosshield-server/audit_hash_transition.go` + `bootstrap.go` 갱신, RegisterCheckpointJob 직후 1 Tx 안 idempotent emit) + `sqliterepo.Repo.transitionSeq atomic.Int64` cache + Set/Get/Find/recompute 메서드 4 종 + **Append 경로 hash version 분기** (transitionSeq 0 또는 entry.Seq <= transitionSeq 면 v1, 그 외 v3, transition entry 자체는 v1 hash — chain link 연속성 보장) + **Verify 경로 동일 분기** (recomputeHashForSeq) + `audit.BundleVersionV3 = "v3"` const + `MarshalEntryLineV3` 신규 (v2 wire 호환 엄격, LeaderEpoch는 v3만 노출) + `ExportEntryLine.LeaderEpoch *int64 omitempty` + `ExportSignatureLine.HashVersionTransitionAt *int64 omitempty` 신규 필드 + `audit.ChainExporter.ExportV3` interface + `Repo.ExportV3` 구현 (v2 super-set + transitionAt + LeaderEpoch) + 마이그레이션 0건 (audit_entries.action + key_epoch + leader_epoch + audit_chain_keys 모두 Phase 10.D 결선) + 9 단위 test (transition_test.go 5 + repo_test.go 4) + PG integration test 1 (hash_version_pg_integration_test.go) + 신규 metric `rosshield_audit_chain_hash_version{tenant}` Gauge + `rosshield_audit_chain_hash_version_transition_total{tenant}` Counter.
- `feat(audit-verify)` Phase 11.C-5 fg-verify v3 + 3-tier backward compat (`cd13b4a`) — `exportOutput.BundleVersion` enum 확장 (`"v1" | "v2" | "v3"`) + `bundleVersionLabel(wire)` `"v3"` 매핑 + `selectHashFunc(useV3, transitionSeq, entrySeq)` 신규 (sqliterepo.Repo.recomputeHashForSeq 일관) + `verifyHashChainForBundle` (bundle version + transitionSeq 기준 v1↔v3 자동 분기) + `verifyHashVersionTransitionMarker` (`_hashVersionTransitionAt` 와 bundle 안 transition entry seq 일관성 검증) + `buildEpochMap`/`lookupSigningPublicKey` v3 분기 추가 (v2 와 동일한 `_chainKeyEpochs` 사용) + testdata 신규 (`testdata/v3_bundle.ndjson.gz`, deterministic chacha8 seed, 1079B, 5 entries: v1 seq 1~2 + transition marker seq=3 v1 hash + key_rotated seq=4 + v3 entries seq 4~5, `_hashVersionTransitionAt=3` + `_chainKeyEpochs[]` 2 epoch + leaderEpoch HA failover 시뮬) + 단위 test 5 신규 (V3Bundle_RoundTrip + V3Bundle_PostTransitionEntryTampered + V3Bundle_BoundaryHashFunctionMismatch + V3Bundle_TransitionAtTampered + V3Bundle_SignatureTampered) + `TestdataFixtures_PASS` 에 v3 case 추가 (회귀 가드) + v3 hash 함수 reimplement 회피 (audit.ComputeEntryHashV3 재사용, 단일 source of truth) + 외부 빌드 일관 (stdlib + crypto/ed25519 + audit 도메인 only, Stage 10.D-5 패턴 cargo cult) + `docs/operations/audit-verify-cli.md` v3 절차 갱신 (3-tier 표 + v3 features + D-P11C-4 분배 시점).
- `docs(release)` Phase 11.C-6 v0.13.0 release notes + CHANGELOG entry (본 release commit) — `docs/releases/v0.13.0.md` 신규 + 본 entry.
...
- 코드 path · API 시그니처 · DB schema · 환경 변수 default 변경 0 — 외부 호출자 영향 0 (Append/Export/Verify 모두 signature 동일). v2 wire 변경 0 (MarshalEntryLine 은 LeaderEpoch 절대 미노출 유지, v3 는 MarshalEntryLineV3 별 변종).

### Verification
- `go vet ./...` PASS · `go build ./internal/... ./cmd/...` PASS · `go test -count=1 ./internal/domain/audit/...` PASS (canonicalMetaJSONv3 + transition + ExportV3 + v1 회귀 0).
- `go test -count=1 -short ./cmd/rosshield-server/...` PASS (bootstrap transition emit 1회 idempotent).
- `go test -count=1 ./cmd/rosshield-audit-verify/...` PASS (v1 + v2 + v3 fixture 모두 PASS + 변조 4종 FAIL 정확).
- `go test -tags=integration -count=1 ./internal/domain/audit/sqliterepo/...` PASS (TestHashVersionTransition_PG + TestExportV3_PG_Wire wire 검증).
- 회귀 0 — Phase 0~11 옵션 C Stage 11.C-5 단위 + 통합 test 모두 그대로 PASS (50+ 패키지). v2 fixture round-trip byte-identical 보존.

### Notes
- **minor bump 이유** — Phase 11 두 번째 minor (Phase 11 첫 minor v0.12.0 옵션 B 후속). 신규 hash 함수 (ComputeEntryHashV3) + 신규 audit action (audit.chain.hash_version_changed) + 신규 bundle version (v3) + 신규 metric 2 + fg-verify v3 3-tier backward compat = customer-facing 신규 기능 묶음.
- **customer 영향 0** — 신규 v0.13.0 deploy 시 bootstrap 이 transition marker 1회 자동 emit (idempotent, 두 번째 부팅부터 noop). 운영자 알람 없음 (INFO 로그 + Prometheus counter). v0.12.0 이하 customer 가 v0.13.0 deploy 시 마이그레이션 0건 (DB schema 변경 0) + 기존 audit_entries 변경 0 (append-only) + 첫 부팅 시 자연 transition.
- **v2 wire backward compat 엄격** — MarshalEntryLine 은 LeaderEpoch 절대 미노출 유지, v3 는 MarshalEntryLineV3 별 변종. v0.10.0~v0.12.x customer 의 v2 bundle 검증 회귀 0.
- **fg-verify v1/v2/v3 3-tier 자동 분기** — `_bundleVersion` 자동 감지, 호출자 옵션 변경 0. v1 bundle (v0.9.0 이하) + v2 bundle (v0.10.0~v0.12.x) + v3 bundle (v0.13.0+) 모두 PASS.
- **Phase 11 진척** — Top 3 순차 B → C → A 진행 중. 옵션 B (SOC2 readiness · v0.12.0) ✅ 마감. 옵션 C (audit hash chain key_epoch + leader_epoch input + fg-verify v3 · v0.13.0) ✅ 마감. 옵션 A (OpenTelemetry tracing 전면 · v0.14.0) 🔵 진입 가능.
- **v0.10.0 carryover 완전 마감** — "audit hash chain key_epoch + leader_epoch input 미포함" 항목 마감. canonicalMetaJSONv3 (9 키) 안에 두 epoch 모두 hash input 포함 + chain transition entry 명시 + fg-verify v3 자동 검증.
- **신규 carryover** — fg-verify v3 binary 외부 감사인 분배 (★ 외부 트랙, D-P11C-4 = 본 release 후 시점) / v0.12.0 이하 bundle 은 v1/v2 fixture 만 (외부 감사인 환경에서 v3 binary 필요) / LeaderEpoch 는 HA 비활성 site 에서 항상 nil (chain integrity 영향 0, v3 hash noop) / audit_chain_hash_version metric Grafana panel 별 dashboard epic.

### Migration
- 신규 v0.13.0 deploy 시 bootstrap 이 audit.chain.hash_version_changed transition marker 1회 자동 emit (RegisterCheckpointJob 직후, 1 Tx 안, idempotent — 두 번째 부팅부터 noop).
- transition 이후 audit entries 는 v3 hash + chain link 자동. 기존 entry (transition 이전) 는 v1 hash 그대로 보존 (append-only 일관, 설계서 §1.9).
- 외부 감사인 fg-verify v3 binary 분배는 본 release artifact 에서 다운로드 — fg-verify v3 는 v1/v2/v3 자동 판별 (호출자 옵션 변경 0). fg-verify v2 는 v1/v2 만 cover, v3 bundle 검증 시 "unknown bundleVersion" error.

---

## [0.12.0] — 2026-05-22 (minor — Phase 11 옵션 B SOC2 readiness 마감)

> **요약**: Phase 11 옵션 B "SOC2 Type II readiness baseline" 완전 마감 + Phase 11 첫 minor release (Phase 11 Top 3 순차 B → C → A 진척 중, v0.12.0 옵션 B 1순위 마감). `docs/compliance/` scaffold + SOC2 Trust Services Criteria 14 control 매트릭스 docs (CC1~CC9 9 + A1~A5 5, 총 **47 sub-control** 매핑, A6 Custodial 미적용) + `auditor` read-only role 활성 (기존 RBAC 매트릭스 재사용, 신규 role 정의 0) + audit log export wizard (`POST /api/v1/compliance/export` + `/compliance/export` UI) + effectiveness dashboard (`GET /api/v1/compliance/effectiveness` + `/compliance/effectiveness` UI, 14 카테고리 × 40 sub-control × audit action 매핑 자동 집계) + `soc2-controls` 자동 검증 pack 신규 (**61 check** CIS-style yaml, CC1~CC9 + A1~A5 8 카테고리 cover) + i18n compliance.* ko+en ~70 신규 키 한 set. 외부 SOC2 firm 진입 baseline 정착 — 본 release는 자체 인증을 의미하지 않습니다. Lodestar 결선 자산이 SOC2 TSC ~95% 자연 cover. 회귀 0, Breaking 0. customer 영향 0 (신규 페이지·endpoint·pack, 기존 변경 0). 상세는 [docs/releases/v0.12.0.md](docs/releases/v0.12.0.md).
>
> **기준 commit**: (본 release commit, main)

### Added
- `design(phase11-b)` Phase 11 옵션 B Stage 분해 + 결정 항목 (`dac77e5` + `c9a2b30`) — `soc2-readiness-design.md` 신규 (13 섹션 + 4 옵션 비교) + D-P11B-1·2·3·4 결정 확정 (옵션 A 일괄 / 전체 5 A 카테고리 / auditor 신규 role / 자동 검증 pack).
- `docs(compliance)` Phase 11.B-2 docs/compliance/ scaffold + SOC2 CC1~CC4 매핑 (`d66bdce`) — `docs/compliance/README.md` 신규 (SOC2 Type II readiness baseline 인덱스 + Lodestar 결선 자산 매핑) + soc2/cc1-control-environment.md (5 sub) + cc2-communication-information.md (3 sub) + cc3-risk-assessment.md (4 sub) + cc4-monitoring-activities.md (2 sub). 5 신규 docs · 747줄. 결선 ~10.5/14 cover (~75%).
- `docs(compliance)` Phase 11.B-3+4 SOC2 CC5~CC9 + A1~A5 매핑 (`c76250e`) — soc2/cc5-control-activities.md (3 sub) + cc6-logical-physical-access.md (8 sub) + cc7-system-operations.md (5 sub) + cc8-change-management.md (1 sub) + cc9-risk-mitigation.md (2 sub) + a1-availability.md (3 sub) + a2-confidentiality.md (2 sub) + a3-processing-integrity.md (4 sub) + a4-privacy.md (8 sub, SOC2 baseline만) + a5-security.md (cross-reference). 10 신규 docs · 1,965줄. 누적 14 매트릭스 docs · 47 sub-controls 매핑 완비. AICPA SOC2 TSC 본문은 의역 (license 회피).
- `feat(auth)` Phase 11.B-5 auditor role + audit log export wizard (`bc5826b`) — `auditor` read-only role 활성 (RBAC 매트릭스 재사용, 신규 role 정의 0) + `POST /api/v1/compliance/export` admin + auditor 엔드포인트 + `internal/api/handlers/compliance_export.go` + `audit.ChainExporter` 도메인 interface 신규 (`sqliterepo.Repo` 자동 만족) + `handlers.Deps.AuditExporter` / `AuditChainKeys` / `AuditSigner` 옵트인 게이트 + `audit.compliance.export` event 동일 Tx emit (entry seq → 응답 헤더) + `/compliance/export` UI route + i18n compliance.export.* (ko + en) + Sidebar nav 항목 (system.read gate, admin + auditor visible) + rbac_integration_test endpoint count 29 → 30 + 단위 test (backend 503/401/400/200(v1+v2)/empty body + web vitest exportErrorMessage 분기 + downloadAuditBundle fetch/blob mock).
- `feat(compliance)` Phase 11.B-6 effectiveness dashboard `/compliance/effectiveness` 페이지 (`c2d53a6`) — `GET /api/v1/compliance/effectiveness` auditor + admin 게이트 (audit.export 권한) + `internal/domain/compliance/soc2_mapping.go` 신규 (14 카테고리 × 40 sub-control × audit action 매핑, 416줄) + `audit.EffectivenessAggregator` interface + `audit/sqliterepo.CountActionsByWindows` 구현 (read-heavy 단일 query, tenant scope index 활용) + `/compliance/effectiveness` UI route (cover% hero + 매트릭스 표 + gaps 카드) + i18n compliance.dashboard.* ko + en (40 신규 키) + Sidebar nav 항목 + 26 신규 test (soc2_mapping 5 + audit/sqliterepo effectiveness 3 + handlers 3 + web vitest 15).
- `feat(packs)` Phase 11.B-7 soc2-controls 자동 검증 pack 신규 (`b57ea29`) — `packs/soc2-controls/pack.yaml` v0.1.0 (**61 check** CIS-style yaml) + CC1~CC9 + A1~A5 8 카테고리 cover (CC6 15 · CC7 10 · CC8 5 · CC1 2 + CC2 3 + CC3 3 + CC4 2 + CC5 3 + CC9 2 = 15 · A1 5 + A2 3 + A3 2 + A4 1 + A5 5 = 16) + 61 selftest fixture (PASS + FAIL + site policy skip 3 case 표준) + `internal/domain/benchmark/soc2_controls_fixture_test.go` (round-trip 회귀 검증 + count [50,80] guard). audit cmd 패턴 3종 mix (host-side bash + Lodestar API curl + docs 존재). env 미설정 시 PASS skip 패턴 일관. self-referential check (CC5.1 — 본 pack 자체 설치 baseline 충족).
- `docs(release)` Phase 11.B-8 v0.12.0 release notes + CHANGELOG entry (본 release commit) — `docs/releases/v0.12.0.md` 신규 + 본 entry + `docs/compliance/README.md` TOC 갱신 (Stage 11.B-5·6·7 자산 명시).

### Changed
- 코드 path · API 시그니처 · DB schema · 환경 변수 default 변경 0 — 신규 페이지·endpoint·pack·role 묶음, 기존 기능 변경 0.

### Verification
- `go vet ./...` PASS · `go build ./internal/... ./cmd/...` PASS · `go test -count=1 ./...` PASS (회귀 0).
- `go test -count=1 -run "TestSOC2ControlsFixtures" ./internal/domain/benchmark/...` PASS — 61/61 check + 61/61 selftest fixture round-trip + 123/123 yaml syntax (61 check + 61 selftest + 1 pack.yaml).
- `pnpm exec tsc -b --noEmit` PASS · `pnpm test --run` 561+ tests PASS (Stage 11.B-5 export wizard + 11.B-6 dashboard 신규 33 test 포함) · `pnpm build` PASS.
- RBAC 회귀 — `rbac_integration_test` endpoint count 29 → 30 (audit.export 1건 추가), 기존 30 endpoint × 6 페르소나 = 180 case 회귀 0.

### Notes
- **minor bump 이유** — Phase 11 첫 minor (Phase 10 옵션 A·D·E v0.9.0·v0.10.0·v0.11.0 후속). docs/compliance/ 14 매트릭스 + auditor role 활성 + 신규 endpoint 2 + 신규 페이지 2 + 신규 pack 1 (61 check) = customer-facing 신규 기능 묶음.
- **customer 영향 0** — 신규 페이지·endpoint·pack은 옵트인 활성. 기존 customer 자동 변경 0. soc2-controls pack은 customer scan profile 등록 시에만 활성 (env 미설정 시 PASS skip 패턴 일관).
- **외부 SOC2 firm 트랙 ★** — 본 release는 baseline까지, 실 인증은 AICPA 회원 외부 firm (Deloitte · KPMG · PwC · BDO · A-LIGN · Coalfire 등) 결정. 90일 운영 효과성 측정 + awareness training + pen test + vendor inventory 모두 외부 트랙.
- **Phase 11 진척** — Top 3 순차 B → C → A 진행 중. 옵션 B (SOC2 readiness · v0.12.0) ✅ 마감. 옵션 C (audit hash chain key_epoch + leader_epoch input · v0.13.0) 🔵 진입 가능. 옵션 A (OpenTelemetry tracing 전면 · v0.14.0) 🔵 별 epic.
- **신규 carryover** — 외부 SOC2 firm + 90일 측정 (★) / awareness training (★) / vendor inventory + pen test (★) / A4 Privacy GDPR/CCPA/한국 PIPA 상세 매핑 (별 epic) / soc2-controls pack archive embed (PACKS_SOURCE Makefile 등록 별 patch) / DC physical (CC6.4) + environmental (A1.3) IaaS cloud SOC2 inheritance 매핑 (별 epic) / formal risk register + fraud risk assessment (internal governance) / formal IR program (별 epic).
