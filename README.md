...
- **자동 변환된 CIS pack**: `packs/cis-ubuntu-2404/` (295 checks 자동 변환, 17 Manual + 0 NoMarker degraded marker)
...
- **전체 시스템 설계서**: [`docs/design/`](./docs/design/) 13개 마크다운 + [`docs/design/notes/`](./docs/design/notes/) 29개 deepdive (🎉🎉 Phase 5 5 epic + Phase 6 후보 1 customer onboarding 모두 마감 — enterprise PoC partner 모집 baseline 완성)
...
| D7 | 초기 타깃 벤치마크 | ✅ CIS Ubuntu 24.04 (자동 변환 94.6% — NoMarker 0건 + Manual 자동 후보 cover) + ROS2 Jazzy baseline |
...
#   total: 312, auto-converted: 295 (94.6% auto)
#   degraded: Manual=17, NoMarker=0 (E-4 Manual 자동 후보 4건 cover, 잔여 17 fixture 트랙)

# tar.gz 서명 + built-in embed
make pack-archive  # bin/pack-tools archive로 packs/*.tar.gz 생성 + internal/builtin/packs/_archives/ 복사
```

**자동 변환 패턴 22종** — 본 세션 18.9% → 94.6% 진척(+75.7%p, 3일):
...
22. **E-4 Manual 자동 후보 4종** (4건) — 1.2.2.1 apt -s upgrade ^Inst 카운트 + 3.1.1 IPv6 disabled hashbang + 4.3.3 iptables/ip6tables -L user rule 0건 + 6.2.3.21 augenrules --check "No change" — Manual 분기 우회로 deterministic 합성 (인식기 OR + audit phrase·cmd 조합 AND)

**NoMarker 0건 완전 마감 + Manual 자동 후보 cover** — 본 세션 56 NoMarker → 0 + Manual 21 → 17(자동 후보 4건). 잔여 degraded 17건은 모두 `assessment_status="Manual"` 중 fixture 트랙 대상(인터랙티브 prompt / multi-step shell / runtime 환경 의존). **Manual fixture Stage 1+2 완료** — 12건(high 3 + medium 9: 1.1.1.10 / 1.2.1.1 / 2.1.22 / 4.2.5 / 4.3.7 / 4.4.2.3 / 4.4.3.3 / 5.3.3.2.3 / 5.4.1.2 / 6.1.1.2 / 6.1.1.3 / 7.1.13)에 운영자 정성 검토 prompt 적용 (`op="manual"` + `defaultVerdict=review`), `packs/cis-ubuntu-2404/checks/manual/` + `selftest/manual/`에 작성. pack-tools convert가 manual 서브디렉토리 round-trip 보존. 운영자 가이드 [`docs/operations/cis-ubuntu-2404-manual.md`](./docs/operations/cis-ubuntu-2404-manual.md). Stage 3(low 5건)은 first paying customer 또는 enterprise 요청 시 진입.

**합성 bash 회귀 검증**: `tags=cis_synth_integration` 옵트인 28 케이스(기존 24 + D Stage 3의 4 auditctl) — CI ubuntu-latest job으로 PR마다 자동 차단. E-4 4건은 selftest YAML로 회귀 보강.
