...
> **CIS Ubuntu 24.04 자동 변환률**: **77.6%** (242/312, 2026-05-13). nrobotcheck baseline JSON에서 9 패턴(PASS 마커 / "Nothing returned" 변형 / stat 권한 / sshd boolean·numeric·range / multi-line cmd 흡수 / hashbang body wrap / grep verify / awk exact match) 합성 변환.
> **탄생일**: 2026-04-23
...
| D7 | 초기 타깃 벤치마크 | ✅ CIS Ubuntu 24.04 (자동 변환 77.6%) + ROS2 Jazzy baseline |
...
6. multi-line cmd 흡수 (quote-balance 기반 join, dangling `--`/`-` 보강) — CIS 5.1.6/12/15 PCRE regex 분할 cover
7. base64 sub-shell wrap — PASS 마커 부재 hashbang body (1.7.10 / 5.4.2.7~8 / 7.2.x duplicate UID/GID)
8. grep + "verify output matches" / "Output should be similar" — auditd config + PAM 13건
9. awk + "verify that only X is returned" 정확 매칭 (5.4.2.1)

**잔여 49 NoMarker**: 6.2.3.x auditd rules 21건(multi-cmd + 정확 expected output) + Manual에 가까운 항목(iptables/nft/GUI dconf) + 5.4.2.2/3 등. Phase 6 후속 또는 customer 환경 hands-on.

**합성 bash 회귀 검증**: `tags=cis_synth_integration` 옵트인 24 케이스(expect-empty/non-empty/stat-perm/sshd-bool/numeric/range/multi-line cipher/grep verify/awk exact/hashbang body) — CI ubuntu-latest job으로 PR마다 자동 차단.
